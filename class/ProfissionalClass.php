...
class Profissional extends Globals {    
    ...
    public static function getProfissional($key, $value, $retorno = null) {
        $qry = montaQuery("profissional AS A",
                "A.*",
    ...
    public function cadCliente() {
        ...
        $_campos['data_nascimento'] = converteData($_campos['data_nascimento']);        
        ...
        if($insere){
            $id = mysql_insert_id();
            Globals::gravaLog($_campos['user_cad'], 8, null, "Cadastrou o cliente {$_campos['nome']}");
    ...
    public function editCliente($id) {
        ...
        if($altera){
            Globals::gravaLog($_SESSION['id_usuario'], 9, null, "Alterou o cliente {$_campos['nome']} | ID: {$id}");
